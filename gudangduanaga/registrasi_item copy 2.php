...
require_once 'auth.php'; // auth.php sudah require functions.php + session_start + cek login

$pageTitle = 'Registrasi Manage';

// ==========================
// Helper Normalisasi Nama PT/CV
// "PT. Dua Naga Kosmetindo" -> "Dua Naga Kosmetindo"
// ==========================
function normalize_company_name(string $name): string {
    $name = trim($name);
    if ($name === '') return '';

    // hapus prefix PT / CV (dengan/ tanpa titik)
...
$userRole = $currentUser['role'] ?? 'user'; // admin / reg / user / inout (bebas, yang penting admin vs non-admin)

// ==========================
// 0A) Tentukan perusahaan user (AUTO FILTER)
// - prioritas: warehouse_id -> lookup ke warehouses
// - fallback: company_name / warehouse_name / company / name
// ==========================
$userCompanyRaw = '';

// 1) via warehouse_id (paling ideal)
...
// Alias kalau ada penulisan beda (misal typo "Farmapo" vs "Farma")
$aliases = [
    'Phytomed Neo Farma'    => 'Phytomed Neo Farmapo',
    'Phytomed Neo Farmapo'  => 'Phytomed Neo Farmapo',
];
if (isset($aliases[$userCompanyNorm])) {
    $userCompanyNorm = $aliases[$userCompanyNorm];
}

// ==========================
// 1) Baca filter perusahaan dari GET
// ==========================
$selectedCompany = normalize_company_name(trim($_GET['company'] ?? ''));

// ==========================
// 1A) KUNCI FILTER PERUSAHAAN UNTUK NON-ADMIN
// - Admin boleh pilih perusahaan (pakai dropdown)
// - Non-admin otomatis pakai perusahaan miliknya (tidak bisa ganti)
// ==========================
$forceCompany = false;
if ($userRole !== 'admin') {
    if ($userCompanyNorm !== '') {
...
// ==========================
// 2) Konfigurasi RFID Reader (Multi Gudang)
// ==========================
$RFID_READERS = [
...
// ==========================
// 2A) Batasi Reader berdasarkan perusahaan user (NON-ADMIN)
// ==========================
$companyReaderMap = [
    'Zweena Adi Nugraha'   => ['zweena_reg'],
    'Dua Naga Kosmetindo'  => ['dnk_gambiran_reg', 'dnk_teblon_reg'],
    'Phytomed Neo Farmapo' => [], // kalau nanti ada reader khusus, isi di sini
    'Indo Naga Food'       => [], // kalau nanti ada reader khusus, isi di sini
];
...
if ($userRole !== 'admin' && $selectedCompany !== '') {
    ...
    if (is_array($mapped)) {
        // hanya ambil yang memang terdaftar di $RFID_READERS
        $mapped = array_values(array_filter($mapped, fn($k) => isset($RFID_READERS[$k])));
        if (!empty($mapped)) {
            $visibleReaderKeys = $mapped;
        }
        // kalau mapping kosong/belum ada, fallback: tetap tampilkan semua (biar tidak “buntu”)
        // tapi akan muncul warning di UI di bawah
    }
}

$visibleReaders = [];
foreach ($visibleReaderKeys as $k) {
    $visibleReaders[$k] = $RFID_READERS[$k];
}

$selectedReader = trim($_POST['reader'] ?? $_GET['reader'] ?? ($visibleReaderKeys[0] ?? 'zweena_reg'));
if (!isset($RFID_READERS[$selectedReader])) {
    $selectedReader = $visibleReaderKeys[0] ?? 'zweena_reg';
}
// jika user non-admin, pastikan reader yang dipilih termasuk yang visible
if ($userRole !== 'admin' && !isset($visibleReaders[$selectedReader])) {
    $selectedReader = $visibleReaderKeys[0] ?? $selectedReader;
}
...
// Kumpulkan daftar perusahaan dari API (dinamis) -> pakai versi normalisasi agar konsisten
$companyOptions = [];
foreach ($salesOrdersAll as $order) {
    ...
    if ($companyNameNorm !== '') {
        $companyOptions[$companyNameNorm] = $companyNameNorm; // unique
    }
}

// Terapkan filter perusahaan ke data sales order (pakai normalisasi)
if ($selectedCompany !== '') {
...
// ==========================
// 4) Handle Form Submit (LOGIKA PHP TETAP SAMA)
// ==========================
$error = '';
        ...
        if (empty($tags)) {
            ...
            if (!empty($dupInForm)) {
                ...
                if (!empty($existingTags)) {
                    ...
                    // kalau non-admin: company tetap pakai perusahaan user (jangan ambil dari GET)
                    $redirCompany = $selectedCompany;
                    header('Location: registrasi_manage.php?success=1&company=' . urlencode($redirCompany) . '&reader=' . urlencode($selectedReader));
                    exit;
                }
            }
        }
...
// ==========================
// 5) Ambil data registrasi (10 Terakhir)
// ==========================
$stmt = $pdo->query("
    SELECT *
            ...
            <div class="card-body">
                ...
                <?php if ($userRole !== 'admin' && $forceCompany && ($companyReaderMap[$selectedCompany] ?? null) === []): ?>
                    <div class="alert alert-warning small">
                        ...
                        Namun reader khusus perusahaan ini belum dipetakan. (Silakan tambahkan di <code>$companyReaderMap</code> bila sudah ada.)
                    </div>
                <?php endif; ?>
                ...
                <form method="get" class="mb-3">
                    ...
                    <?php if ($userRole !== 'admin' && $forceCompany): ?>
                        ...
                        <div class="form-text small">
                            Perusahaan terkunci sesuai hak user.
                        </div>
                    <?php else: ?>
...
// ==============================
// Kontrol START/STOP + polling RFID
// ==============================
